...
use eba::{
    err_payload, is_result_err, make_envelope, ok_payload, result_error, result_value,
    ActorId, Envelope, EnvelopeId, FinishInfo, IdGenHandle, InboxHandle, Job, JobHost,
    HostParams, RequestSpec, Value,
};
...
pub fn park_job(root: Envelope, begins: Rc<std::cell::Cell<u32>>) -> Job {
    let mut j = Job::new(root);
...
pub fn parked_host(timeout_ms: i64) -> (JobHost, Rc<std::cell::Cell<u32>>) {
    ...
    let host = JobHost::new(HostParams {
        ...
        request_timeout_ms: timeout_ms,
        queue_limit: 0,
        make_job: Box::new(move |root| park_job(root.clone(), b.clone())),
    });
    (host, begins)
}
