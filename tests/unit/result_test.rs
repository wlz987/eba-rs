...
use eba::{
    err_payload, is_result_err, is_result_ok, looks_like_result_envelope,
    make_envelope, ok_payload, result_body, result_error, result_request_id,
    result_value, timeout_body, ActorId, MakeOptions, Registry, Value,
};
...
fn looks_like_needs_id_segment() {
    ...
    let plain = make_envelope("x", Value::Int(1), ActorId("a".into()), MakeOptions {
        id_gen: Some(seq_gen(1)),
        ..Default::default()
    })
    .unwrap();
