...
impl Job {
    ...
    pub(crate) fn reject_idle(&self) -> Result<()> {
        if self.finished || self.parked_flag {
...
impl<'a> JobCtx<'a> {
    ...
    pub fn park(&mut self) -> Result<()> {
        self.job.reject_idle()?;
        if !self.job.inflight.is_empty() || !self.job.deferred.is_empty() {
            return Err(Error::State("park with inflight".into()));
        }
        self.job.parked_flag = true;
        Ok(())
    }
    ...
    fn defer_request(&mut self, spec: RequestSpec) -> Result<EnvelopeId> {
        ...
        let eid = self.host.require_loan()?.gen.borrow_mut().next_envelope_id();
