...
use crate::job::{FinishInfo, Job, JobCtx};
...
impl JobHost {
    pub fn new(params: HostParams) -> JobHost {
        ...
        let patterns_ref: Vec<&str> =
            params.patterns.iter().map(|s| s.as_str()).collect();
        let subscriber =
            Subscriber::new(params.actor_id.clone(), &params.inbox, &patterns_ref);
        JobHost {
    ...
    pub fn parked(&self, cause: &EnvelopeId) -> Option<&Job> {
        self.slots.parked_job(cause)
    }
    ...
    pub fn complete_parked(
        ...
        let Some(mut job) = self.slots.take_parked(cause) else {
            return Err(crate::Error::State(format!(
                "parked job not found: {cause:?}"
            )));
        };
        self.set_loan(bus, clock, gen);
        let loan = self.loan.clone().expect("loan just set");
        let res = (|| {
            reply(
                &loan.bus,
                &job.root,
            ...
            let ok = matches!(body.get("ok"), Some(Value::Bool(true)));
            let info = if ok {
                FinishInfo {
                    ok: true,
                    error: None,
                    answer: body.get("value").cloned(),
                }
            } else {
                FinishInfo {
                    ok: false,
        ...
        self.clear_loan();
        if res.is_err() && !job.finished {
            self.slots.place(job);
        }
        res
    }
